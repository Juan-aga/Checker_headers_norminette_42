...
def headernorm():
	...
	users, checked, norm_errors, head_errors = ft_fill_users(users, checked, args)
	if not len(checked):
		print(f'{colors.fail}No .h or .c files to check in {args.path}.{colors.reset}')
		exit()
	if args.headers or head_errors:
		ft_headers_log(checked, args.headers)
	if args.norm or norm_errors:
		ft_norm_log(checked, args.norm)
	ft_create_separator("Headers")
	if not len(users) < 2:
		print(f'{colors.yellow}{len(checked) - head_errors} of {len(checked)} files was edited by {len(users)} users: {", ".join(list(users))}{colors.reset}')
	elif len(users):
	...
	if not norm_errors:
		...
		print(f'{colors.fail}Norminette errors in {norm_errors} of {len(checked)} files.{colors.reset}')
...
def ft_create_separator(msg):
	...
	separator = "-" * size
	string = f'|{colors.yellow}{msg.center(size - 2)}{colors.cyan}|'
